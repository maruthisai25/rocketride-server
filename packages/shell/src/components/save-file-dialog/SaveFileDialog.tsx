 ...
 *
 * Behavior contract (design direction 2026-08-07):
...
/** Characters allowed in file and folder names (matches the pipeline rule). */
const NAME_REGEX = /^[a-zA-Z0-9 _-]+$/;

/** The user-facing explanation of {@link NAME_REGEX}, shown on invalid input. */
const NAME_RULE_MESSAGE = 'Names may use letters, numbers, spaces, hyphens and underscores.';
...
const styles = {
	...
	// Fixed extension adornment — the full filename is always visible.
	nameSuffix: {
		...commonStyles.inputField,
		height: 30,
		display: 'inline-flex',
...
export function SaveFileDialog({ title, vfs, fileTypes, rootLabel = '$/', defaultDir = '', initialName = '', onConfirm, onCancel }: ISaveFileDialogProps): React.ReactElement {
	...
	const nameValid = trimmed.length > 0 && NAME_REGEX.test(trimmed);
	...
	const resultPath = selectedDir ? `${selectedDir}/${trimmed}${activeType.extension}` : `${trimmed}${activeType.extension}`;
	const resultLabel = `${rootLabel}${selectedDir ? `${selectedDir}/` : ''}${trimmed || 'name'}${activeType.extension}`;
	...
	const message = trimmed.length > 0 && !nameValid ? NAME_RULE_MESSAGE : actionError;
		...
		<>
			...
			{overwritePath && (
				<ConfirmDialog
					...
					message={`"${trimmed}${activeType.extension}" already exists - overwrite it?`}
